...
function color(s: string): string {
	...
	const has256 = supportsColor.stdout
		? supportsColor.stdout.has256
		: (process.env.TERM || "").includes("256");
	return has256
		? `\u001B[38;5;104m${s}${deps.ansiStyles.reset.open}`
		: chalk.magenta(s);
}
...
export default class SpinnerAction extends ActionBase {
	...
	private _reset() {
		...
		this._write(
			this.std,
			deps.ansiEscapes.cursorLeft +
				deps.ansiEscapes.cursorUp(lines) +
				deps.ansiEscapes.eraseDown
		);
		this.output = undefined;
	}
